...
if calendar_file and staff_file:
    try:
        ...
        # Add optional columns for scheduling constraints if not present
        ...
        # Validate scheduling constraint columns
        invalid_anti_tag_values = ~staff_df['should_not_be_paired_with_others_with_this_tag'].isin([0, 1])
        if invalid_anti_tag_values.any():
            st.error(f"❌ 'should_not_be_paired_with_others_with_this_tag' column must be 0 or 1 only. Check rows: {staff_df[invalid_anti_tag_values]['name'].tolist()}")
            st.stop()

        # --- NEW COLUMN VALUE VALIDATION ---
        invalid_pro_tag_values = ~staff_df['should_TRY_TO_pair_with_others_with_this_tag'].isin([0, 1])
        if invalid_pro_tag_values.any():
            st.error(f"❌ 'should_TRY_TO_pair_with_others_with_this_tag' column must be 0 or 1 only. Check rows: {staff_df[invalid_pro_tag_values]['name'].tolist()}")
            st.stop()
        ...
        # Show info about scheduling constraints
        staff_with_anti_tag_count = (staff_df['should_not_be_paired_with_others_with_this_tag'] == 1).sum()
        if staff_with_anti_tag_count > 0:
            st.info(f"ℹ️ **Constraint Active (Avoid Pairing):** {staff_with_anti_tag_count} staff member(s) will preferably not be scheduled together on the same day.")

        # --- NEW CONSTRAINT INFO ---
        staff_with_pro_tag_count = (staff_df['should_TRY_TO_pair_with_others_with_this_tag'] == 1).sum()
        if staff_with_pro_tag_count > 0:
            st.info(f"ℹ️ **Constraint Active (Try to Pair):** {staff_with_pro_tag_count} staff member(s) marked to preferably work together. The scheduler will attempt to pair at least two of them if any are scheduled.")
    ...
    with st.expander("ℹ️ How to use this app"):
        ...
        **Optional Scheduling Constraints (Binary 1/0 columns):**
        - `should_not_be_paired_with_others_with_this_tag`: Staff marked with `1` will preferably not be scheduled together on the same day (max 1 per day).
        - `should_TRY_TO_pair_with_others_with_this_tag`: Staff marked with `1` prefer to work together. If any of these staff are available and due for a shift, the scheduler will attempt to pair at least two of them on that day.
        ...
        - ✅ Respects optional "avoid pairing" and "try to pair" constraints.
